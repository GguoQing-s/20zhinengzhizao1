...
import com.example.a20zhinengzhizao1.adapter.Gwcadapter;
...
public class S_GWCActivity extends AppCompatActivity {
    ...
    private void setadapter() {
        ...
        gwcadapter.SetData(new Gwcadapter.SetData() {
            @Override
            public void setdata(int position, String name, int shuliang, int lx) {
                if (lx==1)
                {
                    for (int i=0;i<mgwc.size();i++)
                    {
    ...
    @OnClick({R.id.change, R.id.jt, R.id.jlzf, R.id.qkgwc,R.id.title1})
