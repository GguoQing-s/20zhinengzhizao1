...
import com.example.a20zhinengzhizao1.adapter.Jyqyadapter;
import com.example.a20zhinengzhizao1.adapter.Sjpxadapter;
import com.example.a20zhinengzhizao1.bean.Jy;
import com.example.a20zhinengzhizao1.bean.Jyqy;
...
public class S_Fragment_qiye extends Fragment {
    ...
    private Jyqyadapter mjyqyadapter;
    private Jyadapter mjyadapter;
    private List<Sjpx> msjpx;
    private Sjpxadapter msjpxadapter;
    private String cs;
    private int select = 0, sum = 0,path1;
    private boolean is= false;
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.s_fragment_qiye, null);
                ...
                .setVolleyLo(new VolleyLo() {
                    ...
                    public void onResponse(JSONObject jsonObject) {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            ...
                            huoqu(jsonObject1.optString("csm"));
                        }
                    }
    ...
    private void setadapter() {
        mjyqyadapter = new Jyqyadapter(getActivity(), mjyqy);
        listView.setAdapter(mjyqyadapter);
        mjyqyadapter.SetData(new Jyqyadapter.SetData() {
            @Override
            public void setdata(String csm, String clm, int path) {
                path1=path;
                mjy.clear();
                select = 0;
                sum = 0;
    ...
    private void huoqu(final String csm) {
        VolleyTo volleyTo = new VolleyTo();
        volleyTo.setUrl("get_supplier_material")
                .setVolleyLo(new VolleyLo() {
                    ...
                    public void onResponse(JSONObject jsonObject) {
                        ...
                        for (int i = 0; i < jsonArray.length(); i++) {
                            JSONObject jsonObject1 = jsonArray.optJSONObject(i);
                            if (jsonObject1.optString("cshang").equals(csm))
                            {
                                if (mjyqy.size()==0)
                                {
                                    mjyqy.add(new Jyqy(jsonObject1.optString("name")
                                            , jsonObject1.optString("xh")
                                            , jsonObject1.optString("cshang")
                                            , jsonObject1.optString("cs")
                                            , jsonObject1.optInt("path")));
                                }else {
                                    for (int y=mjyqy.size();y>0;y--)
                                    {
                                        Jyqy jyqy = mjyqy.get(y-1);
                                        if (jyqy.getChangshang().equals(csm))
                                        {
                                            mjyqy.remove(y-1);
                                        }
                                    }
