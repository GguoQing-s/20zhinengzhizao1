        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ContactInfor demoBean = (ContactInfor) objList.get(j);
                    List<String> list = new ArrayList<>();
                    list.add(demoBean.getName());
                    list.add(demoBean.getSex());
                    list.add(demoBean.getBirthDay());
                    list.add(demoBean.getTel());
                    list.add(demoBean.getProduce());
                    list.add(demoBean.getPost());
                    list.add(demoBean.getEmail());
                ...
                Log.d("rrrrrrrrrr", "writeObjListToExcel: "+e.toString());
            } finally {
                if (writebook != null) {
                    try {
                        writebook.close();
